...
if prediction == 1:   # สมมติ 1 = รอดชีวิต
    st.success(f"✅ ผู้ป่วยมีโอกาสรอดชีวิตสูง (ความมั่นใจ {prob[model.classes_==1][0]*100:.2f}%)")
    st.image("./img/12.jpg", width=300)
else:
    st.error(f"⚠️ ผู้ป่วยมีความเสี่ยงสูงต่อการเสียชีวิต (ความมั่นใจ {prob[model.classes_==0][0]*100:.2f}%)")
...
labels_map = {0: "เสียชีวิต", 1: "รอดชีวิต"}
labels = [labels_map.get(c, str(c)) for c in model.classes_]
ax3.bar(labels, prob)
st.pyplot(fig3)
